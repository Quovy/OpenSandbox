...
| R6 | Source and Sink implementations are extensible at compile time. The common pipeline treats AckToken, SourcePositionToken, and EndToken as opaque so future Sources are not bound to file-byte offsets. Sources that need new identity indexes may extend the Store. | Must Have |
...
type SourcePositionToken struct {
    ...
    Source    string
    StreamRef StreamRef
    Value     []byte // Source-defined ordered boundary
}
...
type StreamEnd struct {
    StreamRef         StreamRef
    EndToken          EndToken
    TerminalWatermark *SourcePositionToken // nil only for an empty ordered domain
    Revision          uint64
...
emits StreamEnd only after it has emitted every Delivery and fixed every Source-internal result
through `TerminalWatermark`. `AcknowledgeEnd` rejects until those results are durably committed.
Pipeline and Sink treat all token Values as opaque. Pipeline never compares a SourcePositionToken;
when it receives StreamEnd, per-stream FIFO lets it flush and acknowledge every earlier Delivery
before Finalize.

Factories receive read-only configuration, the Sandbox Store, and a transactional state namespace
scoped to that component. Extensions cannot access the process-wide bbolt handle or another
component's bucket. Sources and Sinks declare compatible Record kinds. Registration is compile time
through explicit packages and imports; v1 has no dynamic loading.
...
and every ordered result through the terminal watermark is complete. A nil watermark is valid only
when the ordered domain is empty, for example a stream containing only CoverageGapRecords. API cache
staleness prevents directory disappearance from proving end. Revision N+1 is created only after N
fully Finalizes and AcknowledgeEnd completes. Data discovered while N is finalizing is persisted as
`late_pending`; if it cannot later be reopened by fingerprint, it becomes a `late-after-finalize`
Gap.
...
1. Resolve frozen sandbox identity by Pod UID. A bounded race may wait for Store update; a record without `sandbox_id` cannot enter a durable target.
...
3. Group the current flush window into transient Batches by StreamRef. All accumulator bytes count against the one global budget. Ready streams are scheduled fairly.
...
When Pipeline receives StreamEnd, per-stream FIFO guarantees that every Delivery through the opaque
terminal watermark was received earlier. Pipeline flushes and acknowledges all work already held by
that stream worker, then persists a finalize intent containing target ID, deterministic FinalizeID,
Revision, and a FinalizedAt fixed on first attempt. It calls Sink Finalize, persists Pipeline
finalized state, and calls idempotent `Source.AcknowledgeEnd`. A crash resumes the same intent. No
step may create different identifiers or start a later Revision.
...
the process alive but starts no Source and reports a specific readiness reason. Credentials may
refresh without changing target identity.
...
| `NODEAGENT_SINK_TIMEOUT` / `NODEAGENT_RETRY_MAX_INTERVAL` | Required backend timeout and maximum jittered retry interval. |
...
- Exercise the Source/Pipeline/Sink contract: complete Record-kind compatibility, per-StreamRef FIFO, opaque token routing without watermark comparison, one in-flight Batch per StreamRef, all-or-retry Consume, Guarantee matching, acknowledgement idempotency, serial Revisions, fixed finalize intent, and shutdown ordering.
